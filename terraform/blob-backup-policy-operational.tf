...
resource "azurerm_data_protection_backup_policy_blob_storage" "blob-backup-policy-operational" {
  name     = "blob-backup-policy-operational"
  vault_id = azurerm_data_protection_backup_vault.backup-vault.id
  #time_zone                        = "Coordinated Universal Time"
  operational_default_retention_duration = "P90D"
...
resource "azurerm_data_protection_backup_instance_blob_storage" "lab-stg-blob-policy-assignment" {
  ...
  backup_policy_id                = azurerm_data_protection_backup_policy_blob_storage.blob-backup-policy-operational.id
